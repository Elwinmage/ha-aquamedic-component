...
Controlla le tue pompe di movimento Aqua Medic da Home Assistant tramite l'API cloud Gizwits.
...
| Dispositivo | Nome interno | Chiave prodotto | Supportato |
|---|---|---|---|
| Aqua Medic EcoDrift / SmartDrift x.1 / x.3 | `Current_Pump` | `63632f4902094055ab3fd994c0d612fa` | ✅ |
| Aqua Medic DC Runner (pompa di ritorno) | `DC_Runner` | `8879684725d14066922374e50889f893` | ❌ |
...
Tutti questi dispositivi utilizzano la piattaforma IoT Gizwits (stesso backend dell'app ufficiale Aqua Medic). Il supporto per ulteriori dispositivi potrà essere aggiunto nelle versioni future.

---

## Entità

Ogni dispositivo SmartDrift / EcoDrift espone le seguenti entità in Home Assistant.
...
| **Modalità controllo 0-10V** | Quando attiva, disabilita il cursore di portata (la pompa è controllata da segnale esterno 0-10V) |
...
| Entità | Descrizione |
|---|---|
| **Aggiorna** | Forza un aggiornamento immediato senza attendere il prossimo ciclo di polling |
...
| Campo | Descrizione |
|---|---|
| **E-mail** | Indirizzo e-mail del tuo account Aqua Medic |
| **Password** | Password del tuo account Aqua Medic |
| **Server Gizwits** | Server regionale — selezionare **Europa** per gli utenti EU |
| **Intervallo di aggiornamento** | Frequenza di polling del dispositivo (5–300 s, predefinito 30 s) |

Il server corretto viene preselezionato automaticamente in base alla lingua di Home Assistant.

Dopo la configurazione, l'intervallo di aggiornamento può essere modificato tramite **Impostazioni → Dispositivi e servizi → Aqua Medic → Configura**.
